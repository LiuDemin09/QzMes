    ...
    return {       
        ...
        init: function () {
           ...
           // $('#txtPlanTime').attr("disabled", "disabled");
            ...
            //工时回车事件
            $('#txtUnitTime').keydown(function (e) {
                // if (e.keyCode == 13) {
                //if (e.which == 13) {
                ...
                '-', {
                    ...
                    handler: function () {
                        ...
                        if (rows.length == 1) {
                            ...
                            $('#txtProductName').val(rows[0].ProductName);
                            $('#gridRoute').datagrid('load', {
                                PartsDrawingNo: rows[0].PARTSDRAWINGNO
                            });
                        }
                        $("#restartDialog").dialog('open'); 
                    }
        ...
        initGridRoute: function () {
            $('#gridRoute').datagrid({   //定位到Table标签，Table标签的ID是grid
                title: '路由列表',
                iconCls: 'icon-view',
                url: '../../services/WsSystem.asmx/QueryDevlopRouteTask',
                width: function () { return document.body.clientWidth * 0.6 },
                height: function () { return document.body.clientHeight * 0.9 },
                nowrap: true,
                fit: false,//true表示分页信息到底，false表示不到底
                remoteSort: false,
                striped: true,//设置为true将交替显示行背景。
                autoRowHeight: false,
                singleSelect: true,
                ...
                columns: [[ //选择                     
                     ...
                     { title: '客户名称', field: 'CustName', width: DevlopTaskSubmit.fixWidth(0.1) },
                     { title: '产品名称', field: 'ProductName', width: DevlopTaskSubmit.fixWidth(0.1) },
                      { title: '工序号', field: 'ProcessNo', width: DevlopTaskSubmit.fixWidth(0.1), sortable: true },
                      { title: '工序', field: 'ProcessName', width: DevlopTaskSubmit.fixWidth(0.1) },
                       { title: '工时', field: 'UnitTime', width: DevlopTaskSubmit.fixWidth(0.1) },
            ...
            $('#gridRoute').datagrid('getPager').pagination({
                pageSize: 7,
                pageNumber: 1,
                pageList: [7,10,20, 30, 40, 50],
                beforePageText: '第',//页数文本框前显示的汉字   
                afterPageText: '页    共 {pages} 页',
                displayMsg: '当前显示 {from} - {to} 条记录   共 {total} 条记录',
                // layout:['refresh']
